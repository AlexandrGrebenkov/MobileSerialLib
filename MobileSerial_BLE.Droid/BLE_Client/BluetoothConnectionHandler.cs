...
namespace AndroidBluetoothLE.Bluetooth.Client
{
    public class BluetoothConnectionHandler
    {
        ...
        public Task<ProfileState> ConnectAsync(BluetoothDevice device)
        {
            var t = new TaskCompletionSource<ProfileState>();
            Connect(device, (s) => t.TrySetResult(s));
            return t.Task;
        }
        ...
        private void OnConnectionStateChanged(BluetoothGatt gatt, GattStatus status, ProfileState newState)
        {
            switch (newState)
            {
                ...
                case ProfileState.Disconnected:
                    ...
                    OnDisconnection();
                    _onConnection(ProfileState.Disconnected);
                    break;
